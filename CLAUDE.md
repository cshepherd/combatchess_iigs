...
Lives in `src/tables.s`, `src/board.s`, `src/line.s`, `src/units.s`, `src/rng.s` and the includes that will follow them (movement, fire, turns), included by GAME and TEST in that order. Conventions:
